        ...
        for size in batch_sizes:
            ...
            for _ in range(args.runs):
                ...
                tt, _, _, _ = measure(
                    ...
                    params,
                    device,
                    max_new_token_override=1,
                )

                ttft_list.append(tt)

                lat, tot, new, kv_mb = measure(
                    model,
                    params,
                    device,
                    measure_kv=True,
            ...
            print(
                f"[{size}] "
                f"avg_ttft: {avg_ttft:.4f} | "
                f"avg_latency: {avg_lat:.4f} | "
                f"tokens/s(total): {tokps_total:.2f} | "
                f"tokens/s(new): {tokps_new:.2f} | "
                f"kv_cache(MB): {avg_kv:.1f}"
            )
